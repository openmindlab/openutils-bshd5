...
public class HibernateDAOPersistenceTest extends AbstractTransactionalTestNGSpringContextTests
{
    ...
    /**
     * warning: in this hibernate version, retrieval of component properties appears to be problematic:
     * 
     * <pre>
     ...
     *     at org.hibernate.criterion.PropertyProjection.toSqlString(PropertyProjection.java:64)
     *     at org.hibernate.criterion.ProjectionList.toSqlString(ProjectionList.java:72)
     ...
     *     at it.openutils.dao.hibernate.ExampleTreeCallback.doInHibernate(HibernateDAOImpl.java:871)
     *     at it.openutils.dao.hibernate.ExampleTreeCallback.doInHibernate(HibernateDAOImpl.java:826)
     *     at org.springframework.orm.hibernate3.HibernateTemplate.doExecute(HibernateTemplate.java:419)
     *     ... 42 more
     * </pre>
     * 
     * for this reason, component properties have been commented out of the test. More info here:
     * https://forum.hibernate.org/viewtopic.php?f=1&t=974762
     */
    @Test
    public void testFindFilteredProperties()
    {
        ...
        List<Object> foundProperties = personDAO.findFilteredProperties(
            ...
            Integer.MAX_VALUE,
            0,
            Collections.singletonList("birthDate"));

        Assert.assertEquals(foundProperties.size(), 2);

        Assert.assertEquals(foundProperties.get(0), alice.getBirthDate());
        Assert.assertEquals(foundProperties.get(1), bob.getBirthDate());

        foundProperties = personDAO.findFilteredProperties(
            filter,
            Integer.MAX_VALUE,
            0,
            Arrays.asList("id", "birthDate"),
            Order.desc("name.givenName"));

        Assert.assertEquals(foundProperties.size(), 2);

        Object[] bobsProperties = (Object[]) foundProperties.get(0);
        Assert.assertEquals(bobsProperties[0], bob.getId());
        Assert.assertEquals(bobsProperties[1], bob.getBirthDate());
        Object[] alicesProperties = (Object[]) foundProperties.get(1);
        Assert.assertEquals(alicesProperties[0], alice.getId());

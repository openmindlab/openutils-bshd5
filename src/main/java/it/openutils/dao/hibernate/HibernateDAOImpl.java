 ...
 *            Object Key
 */
public abstract class HibernateDAOImpl<T extends Object, K extends Serializable>
		extends HibernateDaoSupport implements HibernateDAO<T, K> {

	/**
	 * @author carone
	 * @version $Id$
	 */
	private final class HibernateCallbackForExecution implements
			HibernateCallback {
		 ...
		 */
		private HibernateCallbackForExecution(T filter, int page,
				int maxResults, Map<String, FilterMetadata> metadata,
				Order[] orderProperties, List<Criterion> additionalCriteria) {
			this.filter = filter;
			this.page = page;
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<T> find(final String query, final Object[] obj,
			final Type[] type) {
		return (List<T>) getHibernateTemplate().execute(
				new HibernateCallback() {
					...
					public Object doInHibernate(Session ses)
							throws HibernateException {
						// hibernate 3
						return ses.createQuery(query).setParameters(obj, type)
								.list();
					}
				});
	}
	...
	public T findFilteredFirst(final T filter, List<Criterion> criteria) {
		return getFirstInCollection(findFiltered(filter, null,
				getDefaultFilterMetadata(), 1, 0, criteria));
	}
	...
	/**
	 * @return This is needed as for
	 *         http://opensource.atlassian.com/projects/spring/browse/SPR-2226
	 */
	@SuppressWarnings("unchecked")
	private HibernateDAO<T, K> getThis() {
		try {
			...
			logger
					.debug("Not running inside an AOP proxy, so no proxy can be returned: "
							+ exc.getMessage());
			return this;
		}
	}
	 ...
	 */
	public List<T> findFiltered(final T filter,
			final Map<String, FilterMetadata> metadata) {
	 ...
	 * {@inheritDoc}
	 */
	public List<T> findFiltered(final T filter, final int maxResults,
			final int page) {
		return findFiltered(filter, getDefaultFilterMetadata(), maxResults,
				page);
	}
	 ...
	 */
	public List<T> findFiltered(final T filter,
			final Map<String, FilterMetadata> metadata, final int maxResults,
			final int page) {
	...
	public List<T> findFiltered(final T filter, final Order[] customOrder,
			final Map<String, FilterMetadata> metadata, final int maxResults,
			final int page) {
		return getThis().findFiltered(filter, customOrder, metadata,
				maxResults, page, new ArrayList<Criterion>());
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<T> findFiltered(T filter, Order[] customOrder,
			Map<String, FilterMetadata> metadata, int maxResults, int page,
			List<Criterion> additionalCriteria) {
		final Order[] orderProperties = customOrder != null
				&& customOrder.length > 0 ? customOrder : this
				.getDefaultOrder();

		return (List<T>) getHibernateTemplate().execute(
				new HibernateCallbackForExecution(filter, page, maxResults,
						metadata, orderProperties, additionalCriteria));
	}
	...
	/**
	 * Return a list of <code>Order</code> object to be used for the default
	 ...
	 * @return Query
	 */
	protected List<?> getNamedQuery(final String name,
			final Serializable[] params, final int maxResults) {
	 ...
	 */
	protected List<?> getNamedQuery(final String name,
			final Map<String, Object> params, final int maxResults) {
		return (List<?>) getHibernateTemplate().execute(
				new HibernateCallback() {
	...
	/**
	 * Returns the default set of FilterMetadata that will be applied to any
	 * query. The default implementation doesn't set any default filter,
	...
	/**
	 * Returns the first object in the collection or null if the collection is
	 * null or empty.
	 * 
	 * @param list
	 *            collection
